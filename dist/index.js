...
function buildAgentSessionKey(action) {
    return `agent:${action.agentId}:linear-agent-session:${action.agentSessionId}`;
}
function getOrCreateActiveAgentSession(action) {
...
function markAgentSessionCancelled(action) {
 ...
 */
function isTokenValid(minValidMs = REFRESH_BEFORE_MS) {
    if (!existsSync(CRED_PATH))
        return false;
    try {
        const raw = JSON.parse(readFileSync(CRED_PATH, "utf8"));
        if (!raw.tokenExpiresAt)
            return false;
 ...
 */
async function refreshOAuthToken(cred, logger) {
 ...
 */
async function getValidAccessToken(logger) {
    if (!existsSync(CRED_PATH))
        return undefined;
    try {
...
function scheduleProactiveRefresh(logger) {
    if (refreshTimer)
...
async function dispatchConsolidatedActions(actions, api, queue) {
    ...
    await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
        ...
        dispatcherOptions: {
            ...
            onError: (err) => {
                api.logger.error(`[linear] Reply error: ${formatErrorMessage(err)}`);
            },
        },
    });
}
function readJojoAgentAccessToken() {
...
async function acknowledgeMention(action, api) {
    if (action.event !== "comment.mention" || !action.commentId)
        return;
...
async function createAgentActivity(agentSessionId, content, api) {
    const accessToken = await getValidAccessToken(api.logger);
    if (!accessToken) {
...
function collectRecentCodexSessionFiles(root, sinceMs) {
    const files = [];
    const stack = [root];
...
function extractFinalTextFromCodexLog(content) {
    let finalText;
...
async function recoverAgentSessionFinalFromLogs(action, startedAtMs, api) {
    const root = join(process.env.HOME ?? "", ".openclaw", "agents", action.agentId, "agent", "codex-home", "sessions");
    ...
    for (let attempt = 0; attempt < AGENT_SESSION_FINAL_RECOVERY_ATTEMPTS; attempt += 1) {
        for (const file of collectRecentCodexSessionFiles(root, sinceMs)) {
            ...
            if (!content.includes(action.agentSessionId ?? ""))
                continue;
...
function flattenActivityNodes(activities) {
    if (!activities || typeof activities !== "object")
        return [];
    const a = activities;
    if (Array.isArray(a?.nodes))
        return a.nodes.filter(Boolean);
    if (Array.isArray(a?.edges)) {
        return a.edges.map((edge) => edge.node).filter((node) => Boolean(node));
    }
    return [];
}
function formatAgentActivities(nodes) {
    const lines = [];
...
function extractLinearUploadUrls(...values) {
    const urls = new Set();
...
async function downloadLinearFile(url, accessToken, logger) {
    try {
...
async function fetchAgentSessionContext(action, accessToken, api) {
    const data = await linearGraphql(accessToken, `query AgentSessionContext($id: String!) {
      agentSession(id: $id) {
        issue {
          id
          identifier
    ...
    const uploadUrls = extractLinearUploadUrls(action.promptContext, action.detail, data.agentSession?.activities, issueAttachments).slice(0, MAX_AGENT_SESSION_MEDIA);
    const mediaPaths = [];
    const mediaTypes = [];
...
async function alignIssueWithAgentBestPractices(action, accessToken, api) {
    if (!action.agentSessionId)
        return;
    try {
        const data = await linearGraphql(accessToken, `query AgentSessionIssue($id: String!) {
        agentSession(id: $id) {
          issue {
            id
            team { id }
        ...
        if (stateType !== "started" && stateType !== "completed" && stateType !== "canceled") {
            const stateData = await linearGraphql(accessToken, `query TeamStartedStatuses($teamId: String!) {
          team(id: $teamId) {
        ...
        if (Object.keys(input).length === 0)
            return;
        await linearGraphql(accessToken, `mutation UpdateIssueForAgent($id: String!, $input: IssueUpdateInput!) {
...
async function dispatchAgentSessionAction(action, api) {
    if (!action.agentSessionId)
        return;
    const active = getOrCreateActiveAgentSession(action);
    if (active.cancelled) {
    ...
    try {
        ...
        const startedBody = action.event === "agent_session.prompted"
            ? "收到你的补充信息了，我继续处理。"
            : "收到，我开始处理。";
        await createAgentActivity(action.agentSessionId, { type: "thought", body: startedBody }, api);
        if (active.cancelled)
            return;
        ...
        await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
            ...
            dispatcherOptions: {
                deliver: async (payload, info) => {
                    if (active.cancelled || info.kind !== "final")
                        return;
                    const text = payload.text?.trim();
                    if (!text || text === "NO_REPLY")
                        return;
                    deliveredFinal = true;
                    await createAgentActivity(action.agentSessionId, { type: payload.isError ? "error" : "response", body: text }, api);
                },
                onError: (err) => {
                    if (active.cancelled)
                        return;
                    api.logger.error(`[linear] Agent Session reply error: ${formatErrorMessage(err)}`);
                },
            },
        });
        if (active.cancelled || deliveredFinal)
            return;
        const recoveredFinal = await recoverAgentSessionFinalFromLogs(action, dispatchStartedAtMs, api);
        if (active.cancelled)
            return;
        if (recoveredFinal) {
            await createAgentActivity(action.agentSessionId, { type: "response", body: recoveredFinal }, api);
            return;
        }
        await createAgentActivity(action.agentSessionId, { type: "error", body: "我这边没有生成可发送的最终回复，请再试一次或直接在评论里补充信息。" }, api);
    }
    catch (err) {
        if (active.cancelled) {
...
async function dispatchAgentSessionCancel(action, api) {
    if (!action.agentSessionId)
        return;
    const active = markAgentSessionCancelled(action);
    if (!active)
        return;
    api.logger.info(`[linear] Agent Session ${action.agentSessionId} stop requested; cancelling ${active.sessionKey}`);
    const core = api.runtime;
    const cfg = api.config;
    const body = "/stop";
    const ctx = core.channel.reply.finalizeInboundContext({
    ...
    await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
        ...
        dispatcherOptions: {
            deliver: async () => { },
            onError: (err) => {
                api.logger.error(`[linear] Agent Session cancel dispatch error: ${formatErrorMessage(err)}`);
            },
        },
    });
}
...
export function activate(api) {
    ...
    queue.recover().then((count) => {
        ...
        api.logger.error(`[linear] Queue recovery failed: ${formatErrorMessage(err)}`);
    });
    ...
    api.on("after_tool_call", async (event) => {
        ...
        core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
            ...
            dispatcherOptions: {
                deliver: async () => { },
                onError: (err) => {
                    api.logger.error(`[linear] Queue wake error: ${formatErrorMessage(err)}`);
                },
            },
        }).catch((err) => {
    ...
    const routeEvent = createEventRouter({
        ...
        agentSessionAgentId: api.pluginConfig?.["agentSessionAgentId"] ?? "linear_jojo_agent",
    });
    const debouncer = api.runtime.channel.debounce.createInboundDebouncer({
        ...
        onError: (err) => {
            api.logger.error(`[linear] Debounce flush failed: ${formatErrorMessage(err)}`);
        },
    });
    ...
    const handler = createWebhookHandler({
        ...
        onEvent: (event) => {
            ...
            if (actions.length === 0) {
                api.logger.info(`[event-router] no actions for ${event.action} ${event.type} (${String(event.data.id ?? event.data.agentSessionId ?? "unknown")})`);
            }
            for (const action of actions) {
                ...
                if (action.type === "cancel") {
                    dispatchAgentSessionCancel(action, api).catch((err) => {
                        api.logger.error(`[linear] Agent Session cancel failed: ${formatErrorMessage(err)}`);
                    });
                    continue;
                }
                if (action.type === "wake") {
                    if (action.agentSessionId) {
                        ...
                        ensureInitialized().then(() => dispatchAgentSessionAction(action, api).catch((err) => {
                            api.logger.error(`[linear] Agent Session dispatch failed: ${formatErrorMessage(err)}`);
                            if (!isAgentSessionCancelled(action)) {
                                createAgentActivity(action.agentSessionId, { type: "error", body: `处理失败：${formatErrorMessage(err)}` }, api).catch((activityErr) => {
                                    api.logger.error(`[linear] Agent Session error activity failed: ${formatErrorMessage(activityErr)}`);
                                });
                            }
                        }));
                        continue;
                    ...
                    acknowledgeMention(action, api).catch((err) => {
                        api.logger.error(`[linear] ACK reaction failed: ${formatErrorMessage(err)}`);
                    });
                    dispatchConsolidatedActions([action], api, queue).catch((err) => {
                        api.logger.error(`[linear] Immediate dispatch failed: ${formatErrorMessage(err)}`);
                    });
                }
                if (action.type === "notify") {
                    queue
                        ...
                        .catch((err) => api.logger.error(`[linear] Notify enqueue error: ${formatErrorMessage(err)}`));
                }
            }
        },
    });
    const routePath = typeof api.pluginConfig?.["webhookPath"] === "string" &&
        api.pluginConfig["webhookPath"].trim()
        ? api.pluginConfig["webhookPath"].trim()
        : "/linear/webhook";
    ...
    api.logger.info(`Linear webhook handler registered at ${routePath} (debounce: ${debounceMs}ms)`);
}
